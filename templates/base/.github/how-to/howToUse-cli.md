...
No template files need to be copied per ticket. All task files are auto-generated by the CLI agents.
...
**What happens:** The CLI fetches the Jira ticket (via Atlassian MCP), reads `pre-context.md` from the ticket folder if present, scans `.github/archive/` for similar issues, drafts the Strategic Contract, and writes it to `workflow/tickets/PROJECT-123/prompt.md` and `workflow/tickets/PROJECT-123/reproduce.md`.

**Gate A — CLI pauses here.** You will be asked:
> "I've drafted the Strategic Contract and Reproduction Guide. Shall I proceed to Gate B (Plan-Agent), or do you want revisions?"

**You do:** Read `prompt.md`. If scope looks right, confirm. If not, say what's wrong — the CLI will redraft.
