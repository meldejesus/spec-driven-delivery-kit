...
Write to `workflow/<TICKET>/spike/findings.md`:
- `## Source` — what you read or ran
- `## What it showed` — 2–4 sentences
- `## Gaps / unknowns` — what you couldn't determine from this source

## Output
When investigation is complete, write `workflow/<TICKET>/spike/spike-output.md` with:
...
Also write `workflow/<TICKET>/spike/explained.md` as the readable front-door summary. It should begin with known quantities:
...
1. `workflow/<TICKET>/spike/scope.md` — the question and boundaries
2. `workflow/<TICKET>/spike/findings.md` — what's been investigated so far
...
```
———
📍 Active spike: PROJECT-123 → workflow/PROJECT-123/spike/
```
